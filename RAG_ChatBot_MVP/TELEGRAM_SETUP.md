...
- **Единая бизнес-логика:** и CLI, и Telegram вызывают один и тот же слой — `rag.answer(question)`. Дублирования нет.
- **Транспорт отделён от логики:** `main.py` и `telegram_bot.py` — только ввод/вывод; RAG (ingest, store, rag) — общий код.
- **Режим запуска** задаётся конфигурацией (`.env` или переменная окружения `RUN_MODE`) или явной командой (`telegram` / `run`).

## Добавленные и изменённые файлы

| Файл | Действие |
|------|----------|
| `config.py` | Добавлены загрузка `.env`, `RUN_MODE`, `TELEGRAM_BOT_TOKEN` |
| `main.py` | Добавлены команды `run` и `telegram`, запуск бота по `RUN_MODE` |
| `telegram_bot.py` | **Новый** — Aiogram 3: роутер, обработчики сообщений, вызов `rag.answer` |
| `.env.example` | **Новый** — шаблон для токена и `RUN_MODE` |
| `.gitignore` | **Новый** — игнор `.env`, venv, __pycache__ |
| `requirements.txt` | Добавлена зависимость `aiogram>=3.0,<4` |
| `README.md` | Описание структуры и раздел 6.6 про Telegram |
